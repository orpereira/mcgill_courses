...
def document_retrieval(vocabulary, idf_values, U, Sigma, VT, query, k):
    ...
    processed_query = [word.lower() for word in query if word.lower() in vocabulary and word.isalnum()]
    ...
    query_vector = tfidf_query @ VT.T

    # calculate the cosine similarity between the query vector and all document vectors
    cos_sims = np.array([cos_sim(query_vector, U[i] @ np.diag(Sigma)) for i in range(U.shape[0])])

    # get the indices of the top k similar documents
    retrieved_doc_indices = list(np.argsort(cos_sims)[::-1][1:k+1])
